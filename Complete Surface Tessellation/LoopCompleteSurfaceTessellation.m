%% Loop Complete Surface Tessellation
...
% Loop Complete Surface Tessallation
for i=1:length(NGRange)
    ...
    for j=1:length(betaRange)
        ...
        for k=1:length(edgeDensityRange)
            ...
            Results=DistanceDependentMatrix(NG,edgeDensity,beta);   
            Results=ModularAnalysis(RangeROIs,Results);
            %% Save the output in the same folder of the script
