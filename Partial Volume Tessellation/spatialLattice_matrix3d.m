function LattMatrix=spatialLattice_matrix3d(numberModules,RewiredMatrix,xcentroid,ycentroid,zcentroid)
for i=1:numberModules             
    for j=2:numberModules         
        ...
        zDist= (zcentroid(i)-zcentroid(j))^2;                         % Calcuate squared distance between z values
        distMatrix(i, j)= sqrt(xDist+yDist+zDist);                    % Calculate total distance using the pythagorean theorem
        distMatrix(j, i)= sqrt(xDist+yDist+zDist); 
    end
end
