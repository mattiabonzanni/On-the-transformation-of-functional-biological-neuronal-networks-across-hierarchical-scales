function [moduleBottom,numberModulesBottom,xcentroidBottom,ycentroidBottom,zcentroidBottom,HiddenNodes]=clusterNodesBOTTOM(numberROIs,xcoord,ycoord,zcoord,NG,p)
%% To compute the module compositions and characteristics 
% Bottom Graph --> y=[0;0.2]
% Input:
...
for i=1:side
    ...
    for j=1:side
        ...
        if length(findnode)>0                            % to apply the following lines if and only if the intersection led to a non-empty result
           ...
           numberModulesBottom=numberModulesBottom+1;               % to update the number of modules with more than one node
           moduleBottom{numberModulesBottom,1}=findnode;            % to store the module composition, namely which nodes belong to which module
           moduleBottom{numberModulesBottom,2}=length(findnode);    % to store how many nodes in each module
        end     
    end
end
for i=1:numberModulesBottom
    xcentroidBottom(i)=median(xcoord(moduleBottom{i,1}));           % median of the x-axis coordinates of the ith module
    ycentroidBottom(i)=median(ycoord(moduleBottom{i,1}));           % median of the y-axis coordinates of the ith module
    zcentroidBottom(i)=median(zcoord(moduleBottom{i,1}));
end
HiddenNodes=find(HiddenNodes);
fprintf('Modules BOTTOM computation completed\n')
end