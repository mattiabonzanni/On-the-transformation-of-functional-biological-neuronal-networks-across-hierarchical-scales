function [moduleRight,numberModulesRight,xcentroidRight,ycentroidRight,zcentroidRight,HiddenNodes]=clusterNodesRIGHT(numberROIs,xcoord,ycoord,zcoord,NG,p)
%% To compute the module compositions and characteristics 
% Right Graph --> x=[0.8;0.1]
% Input:
...
HiddenNodes=[1:NG];
numberModulesRight=0;
nodex=find(xcoord>1-p);
for i=1:side
    ...
    for j=1:side
        ...
        if length(findnode)>0                           % to apply the following lines if and only if the intersection led to a non-empty result
           ...
           numberModulesRight=numberModulesRight+1;               % to update the number of modules with more than one node
           moduleRight{numberModulesRight,1}=findnode;            % to store the module composition, namely which nodes belong to which module
           moduleRight{numberModulesRight,2}=length(findnode);    % to store how many nodes in each module
        end     
    end
end
for i=1:numberModulesRight
    xcentroidRight(i)=median(xcoord(moduleRight{i,1}));           % median of the x-axis coordinates of the ith module
    ycentroidRight(i)=median(ycoord(moduleRight{i,1}));           % median of the y-axis coordinates of the ith module
    zcentroidRight(i)=median(zcoord(moduleRight{i,1}));
end
HiddenNodes=find(HiddenNodes);
fprintf('Modules RIGHT computation completed\n')
end