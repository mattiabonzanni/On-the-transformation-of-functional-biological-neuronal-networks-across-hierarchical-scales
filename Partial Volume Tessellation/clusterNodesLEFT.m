function [moduleLeft,numberModulesLeft,xcentroidLeft,ycentroidLeft,zcentroidLeft,HiddenNodes]=clusterNodesLEFT(numberROIs,xcoord,ycoord,zcoord,NG,p)
...
for i=1:side
    ...
    for j=1:side
        ...
        if length(findnode)>0                           % to apply the following lines if and only if the intersection led to a non-empty result
           ...
           numberModulesLeft=numberModulesLeft+1;               % to update the number of modules with more than one node
           moduleLeft{numberModulesLeft,1}=findnode;            % to store the module composition, namely which nodes belong to which module
           moduleLeft{numberModulesLeft,2}=length(findnode);    % to store how many nodes in each module
        end     
    end
end
for i=1:numberModulesLeft
    xcentroidLeft(i)=median(xcoord(moduleLeft{i,1}));           % median of the x-axis coordinates of the ith module
    ycentroidLeft(i)=median(ycoord(moduleLeft{i,1}));           % median of the y-axis coordinates of the ith module
    zcentroidLeft(i)=median(zcoord(moduleLeft{i,1}));
end
HiddenNodes=find(HiddenNodes);
fprintf('Modules LEFT computation completed\n')
end