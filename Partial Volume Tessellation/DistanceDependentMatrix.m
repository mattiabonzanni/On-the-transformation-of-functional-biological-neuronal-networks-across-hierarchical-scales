...
function Results=DistanceDependentMatrix(NG,beta,edgeDensity,Results);
xcoord=rand(1,NG);                                              % x-axis coordinates between [0;1]
ycoord=rand(1,NG);                                              % y-axis coordinates between [0;1]
zcoord=rand(1,NG);                                              % z-axis coordinates between [0;1]
nodeIds=1:NG;
...
for i=1:1:NG             
    for j=2:1:NG         
        ...
        zDist= (zcoord(i)-zcoord(j))^2;                         % Calcuate squared distance between z values
        distMatrix(i, j)= sqrt(xDist+yDist+zDist);              % Calculate total distance in the 3D space using the Pythagorean theorem
        distMatrix(j, i)= distMatrix(i, j); 
    end
end
longerdistance=max(max(distMatrix));                            % to calculate the max value in the matrix
distMatrix=distMatrix-longerdistance;                           % to have closer nodes with higher weight
distMatrix=distMatrix./min(min(distMatrix));                    % to range every between [0;1]. It follows that we can have the threshold's range [0;1].
distMatrix(1:1+size(distMatrix,1):end) = 0;                     % to remove self-loops  
%% Pruning
Degree=NG*(NG-1);                                               % max number of connections
wt=0.5;                                                         % initial value of wt; the value will be updated until reaching the desired edge density
...
Results.Output.GlobalGraph.SWP = 1 - (sqrt(diff_clus^2 + diff_path^2)/sqrt(2));
